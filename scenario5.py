...
def run(df):
    ...
    joinKeys = [list(arr) for arr in list(chain.from_iterable(
        combinations(types, size) for size in range(1, len(types) + 1)))]
    ...
    for idx, keys in enumerate(joinKeys):
        ...
        # Bayes
        accuracy = naive.run(df, keys)

        if (len(keys) == 1):
            accuracy_weight_naive[keys[0]] = accuracy

        result_naive.append({'idx': idx, 'accuracy': accuracy, 'keys': sorted(
            keys, key=lambda x: accuracy_weight_naive[x], reverse=True)})
